...
export default function EditConge({params}: {params: {id: string}}) {
    ...
    useEffect(() => {
        ...
        const fechCat = async () => {
            try {
                const conge: any = await Api.read(`/api/leaverequest/${params.id}`);

                setConges(conge)
                
                const initialVal = {
                    userId: String(conge.user.id) || '',
leave_type: conge.leave_type || '',
start_date: conge.start_date || '',
end_date: conge.end_date || '',
reason: conge.reason || '',
status: conge.status || '',
request_date: conge.request_date || '',
approval_date: conge.approval_date || '',
                };
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsLoading(true);
            const leaveModel = new LeaveModel(+values.userId, values.leave_type, values.start_date, values.end_date, values.reason, values.status, values.request_date, values.approval_date, +params.id);
            const resp = await Api.update(`/api/leaverequest/${params.id}`, leaveModel);
            if (resp.ok) {
                toast({
                    title: "congé Modifié avec succès",
                });
    ...
    return (
        <main className='flex flex-col space-y-10 mb-10'>
            {/** head */}
            <HeadDetaill title={"Editer un congé"} subtitle={"employees mangement"} />
                ...
                <form className="flex flex-col gap-5 items-center justify-center " onSubmit={formik.handleSubmit}>
                    ...
                    <div className="flex space-x-4">
                        ...
                        <div className=" flex flex-col">
                            ...
                            <Select
                                defaultValue={formik.values.leave_type}
                                onChange={(value) => {
