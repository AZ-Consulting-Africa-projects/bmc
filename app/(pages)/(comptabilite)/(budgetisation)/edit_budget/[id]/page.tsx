...
export default function AddDepartement({params}: {params: {id: string}}) {
    ...
    useEffect(() => {
        ...
        const fechCat = async () => {
            try {
                const budgets: any = await Api.read(`/api/poste/${params.id}`);
                const initialVal = {
                    departementId: budgets.departement.id || '',
                    description: budgets.description || '',
                    budget_allocated: budgets.budget_allocated || '',
                    budget_spent: budgets.budget_spent || '',
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsLoading(true);
            const budgetModel = new BudgetModel(+values.departementId, +values.budget_allocated, +values.budget_spent, +values.budget_remaining, values.period, values.description, +params.id);
            const resp = await Api.update(`/api/budget/${params.id}`, budgetModel);
            if (resp.ok) {
                toast({
            ...
            <div className="border-blue-600 w-full p-10 ">
                <form className="flex flex-col gap-5 items-center justify-center " onSubmit={formik.handleSubmit}>
                    ...
                    <div className=" flex flex-col">
                        ...
                        <Select
                        defaultValue={formik.values.departementId}
                            onChange={(value) => {
