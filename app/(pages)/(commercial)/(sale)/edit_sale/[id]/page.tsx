...
export default function EditSale({params}: {params: {id: string}}) {
    ...
    useEffect(() => {
        ...
        const fechCat = async () => {
            try {
                const post: any = await Api.read(`/api/sale/${params.id}`);
                const initialVal = {
                    name: post.name || '',
                    customerId: post.customer.id,
                    stage: post.stage || '',
                    estimated_amount: post.estimated_amount || '',
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const resp = await Api.create(`/api/sale/${params.id}`, saleModel);
            if (resp.ok) {
                toast({
    ...
    return (
        <main className='flex flex-col space-y-10 mb-10'>
            {/** head */}
            <HeadDetaill title={"Editer une Oportunités de vente"} subtitle={"Gestion du departement commercial"}   />
