...
export default function EditEmployees({params}: {params: {id: string}}) {
    ...
    useEffect(() => {
        ...
        const fechCat = async () => {
            try {
                const users: UserModel = await Api.read(`/api/user/${params.id}`);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            setIsLoading(true);
            if (value != "") {
                const userModel = new UserModel(values.email, values.firstName, values.lastName, Number(values.phone), values.departement, values.position, value, String(user?.password), true, true, String(values.hire_date), String(image), Number(values.salary), +params.id);
