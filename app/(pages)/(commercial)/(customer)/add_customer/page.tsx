...
export default function AddCustomer() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsLoading(true);
            const customerModel = new CustomerModel(values.name, values.company, +values.phone, values.email, values.address, values.status);
            const resp = await Api.create('/api/customer', customerModel);
            if (resp.ok) {
                toast({
    ...
    return (
        <main className='flex flex-col space-y-10 mb-10'>
            {/** head */}
            <HeadDetaill title={"Ajouter une Client"} subtitle={"Organisation mangement"} />
